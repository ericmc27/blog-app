...
const Profile = () => {
  ...
  const { data: currentUserData } = useCustomQuery(["user", id], ()=> getUserData(id))
  const { navigateToBlog } = useCustomNavigate()


  const handlePhotoChange = async (e) => {
    await uploadProfilePicture(e.target.files[0], id);
  };
  ...
  return (
    <div className="flex w-full">
      ...
      <div className="ms-145 flex flex-col items-center gap-6 rounded bg-amber-400">

        <label htmlFor="profilePicture">
          <div className="h-[120px] w-[120px] border rounded-full hover:cursor-pointer mt-5">
            <img className="w-full h-full rounded-full" src={`${import.meta.env.VITE_BACKEND_URL}/static/${currentUserData?.profilePicture}`} />
          </div>
        </label>
        ...
        <div className="flex flex-col gap-6 bg-white px-10">
          {currentUserData?.blogs?.map((blog, index) => {
            return (
              <div
                key={index}
                className={`flex flex-col w-85 h-75 rounded bg-white shadow-2xl ${
                  index === currentUserData.blogs.length - 1 && "mb-5"
                } ${index === 0 && "mt-5"}`}
              >
